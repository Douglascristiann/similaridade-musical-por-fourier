...
def baixar_musicas(lista_de_links, pasta_download):
 ...
 #       'skip_download': False
    ...
    with YoutubeDL(ydl_opts) as ydl:
        for link in lista_de_links:
            try:
                print(f"⬇️ Baixando e reconhecendo: {link}")
                info = ydl.extract_info(link, download=True)
                titulo = info.get('title', 'Título não encontrado')
                artista = info.get('artist') or info.get('uploader') or 'Artista não encontrado'
                arquivo = f"{titulo}.mp3"
                resultados.append({'titulo': titulo, 'artista': artista, 'arquivo': arquivo})
            except Exception as e:
                print(f"❌ Erro ao baixar/reconhecer {link}: {e}")
                resultados.append({
                    'titulo': 'Erro ao obter título',
                    'artista': 'Erro ao obter artista',
                    'arquivo': 'desconhecido.mp3'
                })
