...
async def reconhecer_shazam_trechos(path):
    ...
    trecho_duracao = 15
    passo = 10
    tentativas = max(1, int((duracao_total - trecho_duracao) / passo) + 1)

    print(f"🔎 Analisando {tentativas} trechos para Shazam...")
    ...
    for i in range(tentativas):
        ...
        if result and result.get("track"):
            ...
            album = track.get("sections", [{}])[0].get("metadata", [{}])[0].get("text", "Não Encontrado")
...
async def reconhecer_musica(path):
    ...
    resultado = reconhecer_metadado(path)
    return resultado
...
def recomendar_knn(nome_base, vetor_base):
    ...
    for rank, (i, dist) in enumerate(zip(indices[0], distancias[0]), 1):
        ...
        similaridade = max(0, 100 - dist)  # Aproximação grosseira
