...
def _formatar_generos_para_db(genero_input: any) -> str | None:
    ...
    # Se for uma string no formato de lista (ex: '["rock", "pop"]'), converte para lista
    if isinstance(genero_input, str) and genero_input.startswith('[') and genero_input.endswith(']'):
        try:
            # ast.literal_eval é uma forma segura de converter a string
            lista_generos = ast.literal_eval(genero_input)
        except (ValueError, SyntaxError):
            # Se falhar, usa a string original (removendo os colchetes)
            return genero_input.strip("[]").replace("'", "").replace('"', '')

    # Se o resultado for uma lista, junta os itens com vírgula e espaço
    if isinstance(lista_generos, list):
        return ", ".join(map(str, lista_generos))

    # Se já for uma string ou outro tipo, apenas converte para string
