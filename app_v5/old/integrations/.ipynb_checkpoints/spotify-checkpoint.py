...
# ==============================================================================
# === CORREÇÃO: URLs oficiais e definitivas da API do Spotify ===
# ==============================================================================
...
if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET:
    log.warning("As credenciais SPOTIFY_CLIENT_ID e SPOTIFY_CLIENT_SECRET não foram configuradas.")
...
def _get_token() -> Optional[str]:
    ...
    try:
        ...
        log.error(f"Falha ao obter token do Spotify: {e}")
        return None
...
def _api_get(endpoint: str, params: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        log.error(f"Falha na chamada da API do Spotify para o endpoint '{endpoint}': {e}")
        return None
...
def enrich_from_spotify(artist_hint: str | None, title_hint: str | None, album_hint: str | None, duration_sec: float | None) -> Dict[str, Any]:
    ...
    year = int(release_date.split('-')[0]) if release_date and release_date.startswith(('1', '2')) else None
...
def spotify_flow(title: Optional[str], artist: Optional[str], album: Optional[str]) -> Optional[Dict[str, Any]]:
    log.info(f"Buscando no Spotify por: '{artist} - {title}'")
    try:
        result = enrich_from_spotify(artist, title, album, None)
        if result and result.get("accepted"):
            log.info(f"✅ Spotify encontrou: {result['title']} — {result['artist']}")
            return result
        else:
            reason = result.get('reason', 'unknown_reason')
            log.warning(f"⚠️ Spotify não encontrou um resultado válido. Razão: {reason}")
            return None
    except Exception as e:
        log.error(f"Erro inesperado durante o fluxo do Spotify: {e}")
        return None