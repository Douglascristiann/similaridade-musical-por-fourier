...
SPOTIFY_FEATURES_ENABLED = False
...
db_lookup = DBLookup()  # passe conn se o seu db.py exigir

# carregar X/ids e sua função de vetorizar query
X = ...         # sua feature_matrix (np.ndarray)
ids = ...       # np.ndarray com id_musica
def get_vec_for_query(q: dict): 
    return ...  # sua rotina atual de vetor da query

# knn = KNNRecommender(
#     feature_matrix=X,
#     item_ids=ids,
#     get_vector_for_query=get_vec_for_query,
#     db_lookup=db_lookup,                    # <- AQUI entra o adaptador
#     penalty_engine=build_penalty_engine(),  # ou deixe None pra usar o default do config
# )

query = {"id_musica": id_escolhido}  # ou artist/title/path/genres
...
GENRE_PENALTY_ENABLED      = _as_bool(os.getenv("GENRE_PENALTY_ENABLED"), True)
GENRE_PENALTY_STRICT_LEVEL = int(os.getenv("GENRE_PENALTY_STRICT_LEVEL", "1"))
PENALTY_SHADOW_MODE        = _as_bool(os.getenv("PENALTY_SHADOW_MODE"), True)




# Torna o filtro mais forte
SEM_GENRE_STRICT = True                # obriga pertencer ao whitelist do gênero do query (se houver)
