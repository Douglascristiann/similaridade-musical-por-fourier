    ...
    try:
        ...
        if not md["accepted"]:
            ...
            try:
                ...
                pass
            # Não retorna aqui para garantir a limpeza do arquivo
        else:
            ...
            if recomendar:
                ...
                # Passa os metadados obtidos para a função de recomendação
                recs = recomendar_por_audio(str(arquivo), k=k, sr=sr, excluir_nome=arquivo.name, query_meta=md)
                if not recs:
        ...
        if AUTO_DELETE_DOWNLOADED:
            try:
                ...
                try:
                    is_in_downloads = arquivo_path.is_relative_to(downloads_path)
                except (ValueError, TypeError):
                    is_in_downloads = str(arquivo_path).startswith(str(downloads_path))

                if is_in_downloads:
                    log.info(f"🗑️  Removendo arquivo processado do diretório de downloads: {arquivo_path.name}")
                    sidecar_file = arquivo_path.with_suffix(".info.json")
                    
                    if sidecar_file.exists():
                        try:
                            sidecar_file.unlink()
                        except OSError as e:
                            log.warning(f"⚠️  Não foi possível remover o arquivo sidecar: {sidecar_file}. Erro: {e}")
                            
                    if arquivo_path.exists():
                        try:
                            arquivo_path.unlink()
                        except OSError as e:
                            log.warning(f"⚠️  Não foi possível remover o arquivo de áudio: {arquivo_path}. Erro: {e}")
            except Exception as e:
                log.error(f"❌ Erro inesperado na rotina de limpeza de arquivos: {e}")
